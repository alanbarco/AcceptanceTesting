...
def step_then_todo_list_contains(context, task_name, priority):
    tasks = [str(task) for task in context.todo_list.tasks if task.name == task_name and task.priority == priority]
    assert tasks
...
def step_then_output_should_contain(context):
    for row in context.table:
        tasks = [str(task) for task in context.todo_list.tasks if task.name == row['Task'] and task.priority == row['Priority']]
        assert tasks
...
def step_then_todo_list_should_show_task_as_completed(context, task_name):
    tasks = [str(task) for task in context.todo_list.tasks if task.name == task_name and task.status == 'Completed']
    assert tasks

@when('the user deletes task "{task_name}"')
def step_when_user_deletes_task(context, task_name):
    task_index = next((index for index, task in enumerate(context.todo_list.tasks) if task.name == task_name), None)
...
def step_then_todo_list_does_not_contain(context, task_name):
    tasks = [str(task) for task in context.todo_list.tasks if task.name == task_name]
    assert not tasks
